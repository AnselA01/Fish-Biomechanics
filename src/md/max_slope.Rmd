...
folds <- vfold_cv(pf09cp01short, v = 10)
grid <- grid_regular(num_terms(c(2,15)), levels = 10)

mars_model <- mars(num_terms = tune()) |>
  set_engine("earth") |>
  set_mode("regression")

mars_recipe <- recipe(Stress ~ Strain, data = pf09cp01short)
...
mars_final_fit <- fit(mars_final_wf, pf09cp01short)

augment(mars_final_fit, pf09cp01short) |>
  ggplot() +
