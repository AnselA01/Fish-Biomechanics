...
for (bone in bones) {
  ...
  minScore <- min(result$max.score, result$inflection.score, result$fds.score)
  scores <- list(max = result$max.score, inflection = result$inflection.score, fds = result$fds.score)
  minScoreMethod <- names(scores)[which.min(scores)]

  maxColor <- ifelse(minScoreMethod == "max", "darkGreen", "red")
  inflectionColor <- ifelse(minScoreMethod == "inflection", "darkGreen", "red")
  fdsColor <- ifelse(minScoreMethod == "fds", "darkGreen", "red")
  ...
  if (do.save) {
    image.save(bone, plot, directory = "ymResults/min_score_decisions")
  }
}
```
