...
b_spline_basis <- bSpline(x, df = 10)

model <- lm(Stress ~ bSpline(Strain, df = 10), data = pf09cp01)
bsMat <- bSpline(x, df = 10, intercept = TRUE)
dbsMat <- dbs(x, df = 10, intercept = TRUE)
...
pf09cp01$first_deriv <- dbs(x, df = 10, derivs = 1)                    # the 1st derivaitves
pf09cp01$second_deriv <- dbs(x, df = 10, derivs = 2)                # the 2nd derivaitves

pf09cp01$deriv1 <- deriv(b_spline_basis)

pf09cp01$deriv2<- deriv(b_spline_basis, derivs = 2)
```
...
```{r}
loop(for (bone in data.generator("./data")) {
  bsMat <- bSpline(bone$Stress, df = 10, degree = 3, intercept = TRUE)
