...
```{r}
model <- lm(Stress ~ bSpline(Strain, df = 20), data = pf09cp01)
...
in progress: solving second derivative 0 instead of taking the values straddling 0.
```{r}
second_deriv <- function(x) {
  second_deriv_matrix <- dbs(x, df = 10, derivs = 2)
  return(as.numeric(second_deriv_matrix %*% coefs[-1]))
}
x_short <- pf09cp01short$Strain
x_long <- pf09cp01$Strain

roots <- uniroot(second_deriv, interval = range(x_short))$root # does not work
```
