...
```{r}
# data
results <- read_csv("results/youngs-modulus/2025-04-22/choices.csv")
...
* We have significant evidence that bone type is associated with Youngs Modulus (p=0.0083).

```{r}
TukeyHSD(a1)
```

* There is a significant difference in Youngs Modulus values for UT compared to MT bones (p=0.004).
...
Interpretation: The model with the bone type is significantly better at predicting the E values than the model with just the intercept (p=0.002).
...
Interpretation: The log transformed model with the bone type is significantly better at predicting the E values than the log transformed model with just the intercept (p<0.001).
